...
Feature: Cancel a booking
  ...
  Background:
    ...
    And I set "cancel booking" API endpoint

  Scenario: User successfully cancel a booking
    ...
    When I send "DELETE" request to "cancel booking" API
    Then the API response status should be 200
    And the "cancel booking" API response should match "success" schema
    And the "cancel booking" API response message should be "Booking cancelled"

  Scenario: User cannot cancel a booking that does not exist
    Given I set booking id path parameter to 999999
    When I send "DELETE" request to "cancel booking" API
    Then the API response status should be 404
    And the "cancel booking" API response should match "error" schema
    And the "cancel booking" API response error should be "Booking with id 999999 not found"
